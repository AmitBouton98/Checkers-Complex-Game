...
class Board:
        ...
        for piece in pieces:
            if isinstance(piece, Piece):
                ...
                try:
                    ...
                    # If self.selected_piece is not defined or doesn't have the knight attribute
                    self.board[piece.row][piece.col] = (0, None)
                    if piece.knight:
                        # If the captured piece is a knight, reset its position to -1, -1
                        piece.move(-1, -1)
    ...
    def _knight_moves(self, piece):
        knight_color = SPECIAL_BLUE if piece.color == BLUE else SPECIAL_RED
        moves = {}
        directions = [
            (2, 1), (1, 2), (-1, 2), (-2, 1),
            (-2, -1), (-1, -2), (1, -2), (2, -1)
            ...
            if 0 <= new_row < ROWS and 0 <= new_col < COLS:
                ...
                if target == 0 or (isinstance(target, Piece) and target.color != piece.color and not (piece.color == SPECIAL_RED and target.color == RED) and not (piece.color == SPECIAL_BLUE and target.color == BLUE)):
                    if isinstance(target, Piece) and (target.color == piece.color or (piece.color == SPECIAL_RED and target.color == RED) or (piece.color == SPECIAL_BLUE and target.color == BLUE)):
                        continue
                    if isinstance(target, Piece) and target.color == knight_color:
                        continue
...
def minmax(board, depth, alpha, beta, maximizing_player):
    ...
    if maximizing_player:
        ...
        for move in moves_to_consider:
            ...
            eval, _ = minmax(temp_board, depth - 1, alpha, beta, False)
            if eval > max_eval:
                max_eval = eval
                best_move = move
        ...
        for move in moves_to_consider:
            ...
            piece, move_pos, skipped = move
            temp_board.change_turn()
            eval, _ = minmax(temp_board, depth - 1, alpha, beta, True)
            if eval < min_eval:
                min_eval = eval
                best_move = move
            beta = min(beta, eval)
    ...
    while run:
        ...
        if board.turn == BLUE and not board.setup_phase and not board.winner:
            ...
            if box_position and len(board.blue_boxes) == 0:
                ...
                _, best_move = minmax(board, 3, float('-inf'), float('inf'), True)
                if best_move:
